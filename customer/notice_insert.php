...
if(isset($_FILES["upfile"])){           // 파일이 있을 경우
    ...
    $upload_dir='./data/';         // 데이터를 저장시켜줄 경로를 지정
...
if(isset($mode) && $mode === "modify"){ //글수정
    ...
    $sql="insert into $table (id,name,division,subject,content,regist_day,hit,file_name_0,file_name_1,file_copied_0,file_copied_1)";
    $sql.="values('$id','$name','$division','$subject','$content','$regist_day',0, ";
    for($i=0;$i<$count;$i++){
        if($files["name"][$i]!=""){
            $sql.= "'{$upfile_name[$i]}', ";
        }else{
